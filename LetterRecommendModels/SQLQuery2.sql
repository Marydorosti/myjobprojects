...
SELECT  DISTINCT
        ...
        ur.UserName AS ReceiverUser
FROM    (SELECT TOP (600000)   * FROM  dbo.AUT_WorkFlow WITH (NOLOCK) ORDER BY   NodeId DESC) w
        INNER JOIN dbo.AUT_Objects o WITH (NOLOCK) ON o.Id = w.ObjId
        INNER JOIN dbo.AUT_WorkFlow p WITH (NOLOCK) ON w.ParentId = p.NodeId
